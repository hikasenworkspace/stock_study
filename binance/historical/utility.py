...
def download_file(base_path, file_name, ext_file_name, date_range=None, folder=None, extract = False, po = True ):
    ...
    csv_save_path = get_destination_dir(os.path.join(base_path, ext_file_name), None)
    
    if os.path.exists(save_path):
        print("\nfile already exists! {}".format(save_path)) if po else None
        if extract:
            if not os.path.exists(csv_save_path):
                unzip(save_path)
            else:
                print("\nExtracted file already exists! {}".format(csv_save_path)) if po else None
        return
